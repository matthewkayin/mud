...
app.post('/api/auth/token', async (req: Request<{}, {}, TokenExchangeBody>, res: Response) => {
  const { code, code_verifier, redirect_uri } = req.body;

  if (!code || !code_verifier || !redirect_uri) {
    return res.status(400).json({ error: 'Missing required parameters.' });
  }
  if (!process.env.RC_AUTH_URL || !process.env.RC_CLIENT_ID || !process.env.RC_CLIENT_SECRET) {
    return res.status(500).json({ error: 'Env not configured' });
  }

  console.log(`Auth URL: ${process.env.RC_AUTH_URL}`);

  try {
    const response = await fetch(process.env.RC_AUTH_URL, {
      method: 'POST',
      headers: {
